                ...
                <li class="dropdown notifications-menu">
                    <?php 
                        ...
                        foreach ($notif_count = DB::table('notifikasi')
                                            ...
                                            ->where([
                                                    ['admin_status','unseen'],
                                                    ['isi_notifikasi','!=','Request diterima']
                                                ])
                                            ->get() as $admin_notif_count) {
                            $i++;
                        }
                    ?>
                                ...
                                @foreach ($notif = DB::table('notifikasi')
                                        ...
                                        <li>
                                            <div class="card" style="padding: 5px 10px">
                                                <a href="#">
                                                    @if ($admin_notif->isi_notifikasi == 'Pengajuan pembuatan akun baru')
                                                        <i class="fa fa-users text-aqua"> 
                                                    @elseif($admin_notif->isi_notifikasi == 'Kritik dan Saran Baru')
                                                        <i class="fa fa-pencil"> 
                                                    @elseif($admin_notif->isi_notifikasi == 'Redirect Kritik dan Saran')
                                                        <i class="fa fa-forward text-aqua"> 
                                                    @endif
                                                        {{$admin_notif->isi_notifikasi}}
                                                    </i>
                                                </a>
                                                <div class="pull-right">
                                                    <a href="{{ url('admin/seenNotif/'.$admin_notif->id)}}">seen</a>
                                                </div>
                                            </div>
                                        </li>
                                @endforeach
